...
struct HttpRequest {
  ...
  /**
   * @brief the custom params passed when the router matches uri with regex
   * capture groups. Note that the first element is the entire string. the 1st
   * capture group is in custom_params[1]. the n th capture group is in
   * custom_params[n].
   **/
...
bool parse_http_req_line(HttpParserStringStream &stream,
                         HttpRequest &http_req) {
  return parse_http_method(stream, http_req) &&
         parse_http_uri(stream, http_req) &&
         parse_http_version(stream, http_req);
}
...
bool parse_http_multi_part_header(HttpParserStringStream &stream,
                                  HttpRequest &http_req) {
  std::string key = stream.seek_until(": ");
  std::string val = stream.seek_until("\r\n");
  if (key.empty() || val.empty()) {
...
bool parse_http_multi_part_headers(HttpParserStringStream &stream,
                                   HttpRequest &http_req) {
  while (stream.match("\r\n") == false) {
...
bool parse_http_multi_part_body(HttpParserStringStream &stream,
                                HttpRequest &http_req) {
...
HttpRequestParserError parse_http_req_str(const std::string &http_req_str,
  ...
  bool is_req_multi_part =
      content_type_header_val.compare(0, multi_part_form_data.size(),
                                      multi_part_form_data) == 0;
  if (is_req_multi_part) {
    std::string output_req_body_copy = output_req.body;
    std::string boundary_str =
        str_split_unsafe(content_type_header_val, "=")[1];
    ...
    std::vector<std::string> multi_part_datas =
        str_split_unsafe(output_req_body_copy, split_at);
    for (const auto &multi_part_data : multi_part_datas) {
      ...
      success =
          success && parse_http_multi_part_body(multi_part_stream, output_req);
      if (!success)
