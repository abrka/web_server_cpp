...
enum class SocketRecvHttpStringError {
  ...
  MAX_ITERATIONS_PASSED,     // request couldnt be parsed within max limit of
                             // iterations
};

SocketRecvHttpStringError socket_recv_http_string(
    int sockfd, std::string &output, size_t max_req_size = (10 * 1024 * 1024),
    size_t data_eaten_per_recv = (1024 * 1024), size_t max_iterations = 1000) {
  std::string recv_msg{};
  ...
  for (size_t i = 0; i < max_iterations; i++) {
    ...
    bool success = str_extract_substr_between_delims(
        recv_msg, "Content-Length: ", "\r\n", content_length_str);
...
void socket_send_http_response(int sockfd, const HTTP::HttpResponse &response) {
  ...
  auto bytes_sent = Net::send(sockfd, (void *)http_response_str.c_str(),
                              http_response_str.length());
